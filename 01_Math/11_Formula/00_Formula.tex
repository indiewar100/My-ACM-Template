...
\item pick定理：给定坐标均是整点的简单多边形，设其面积为$S$，内部整点数为$a$，边界上整点数为$b$，那么他们满足关系$S=a+b/2−1$
 一条边上经过的整点数(包括端点)为$b_i=gcd(abs(P0.x−Pt.x),abs(P0.y−Pt.y)+1bi=gcd(abs(P0.x−Pt.x),abs(P0.y−Pt.y))+1$
